...
class EcheancierController extends ApiController
{
    public function show(Request $request, CFDossierRepository $dossierRepo, string $refPret)
    {
        ...
        $contratGroupe = GroupLoanContract::where('ref_pret', $refPret)->firstOrFail();
        $dossier = $dossierRepo->find($contratGroupe->dossier_id);

        $page = GroupSchedule::where('ref_pret', $refPret)->paginate($request->integer('per_page', 20));

        return response()->json([
            'data' => [
                'tiers'       => $tiers,
                'dossier'     => $dossier,
                'sum_montant' => GroupSchedule::where('ref_pret', $refPret)->sum('montant'),
